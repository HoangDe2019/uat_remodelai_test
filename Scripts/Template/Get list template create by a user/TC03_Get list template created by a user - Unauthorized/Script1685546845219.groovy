...
def userEmail = itemlist[itemlist.size()-1].email

'When Call the request to get list of templates with Unauthorized'
ResponseObject responseTemplates1 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/template/list?email=${userEmail}", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseTemplates1, 401)

'When Call the request to get list of templates with Unauthorized'
ResponseObject responseTemplates2 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/template/list?email=${userEmail}", 'Bearer', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseTemplates2, 401)
