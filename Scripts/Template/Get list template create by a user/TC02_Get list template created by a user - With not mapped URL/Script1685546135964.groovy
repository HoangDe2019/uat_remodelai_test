...
'When Call the request to get list of templates with URL not mapped'
ResponseObject responseTemplates = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/templates/list?email==${userEmail}", token, '')

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseTemplates, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseTemplates, "error", 'Not Found')
