...
responseBodyTemplates.each { info ->
	...
	assert info.templateId != null
	info.companyId == itemlist[itemlist.size()-1].companyId
	assert info.displayName != null
	assert info.singleOptionQuestions != null
	assert info.multipleChoiceQuestions != null
	info.userId == itemlist[itemlist.size()-1].userId
	info.email == GlobalVariable.email
}
