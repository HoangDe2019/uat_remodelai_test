...
'When Call the request to get template by emailtemplateId with blank token'
ResponseObject responseTemplate1 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/email/template/${templateId}", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseTemplate1, 401)

'When Call the request to get template by emailtemplateId with invalid token'
ResponseObject responseTemplate2 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/email/template/${templateId}", 'Bearer', '')
