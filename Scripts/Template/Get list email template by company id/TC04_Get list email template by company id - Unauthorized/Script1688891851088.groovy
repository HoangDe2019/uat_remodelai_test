...
'When Call the request to get email templates by companyID with blank token'
def companyId = itemlist[itemlist.size()-1].companyId
ResponseObject responseTemplate1 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v1/email/template/list/company' + companyId, '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseTemplate1, 401)

'When Call the request to get email templates by companyID with invalid token'
ResponseObject responseTemplate2 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v1/email/template/list/company/' + companyId, 'Bearer', '')
