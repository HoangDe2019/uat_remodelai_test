...
'When Call the request to get company with URL not mapped'
def companyId = responseBodyOfComapanies.keySet()
ResponseObject responseCompany = CustomKeywords.'resources.api.buildApiGetMethod'('api/v1/companyinvalid/' + companyId[0], tokenId, '')

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseCompany, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseCompany, "error", 'Not Found')