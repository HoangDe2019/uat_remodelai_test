...
//param request
data = "{\n    \"companyId\": \"${responseBodyOfUsers[responseBodyOfUsers.size()-1].companyId}\",\n    \"name\": \"test email template No.${randomString}\",\n    \"email\": \"${responseBodyOfUsers[responseBodyOfUsers.size()-1].email}\",\n    \"description\": \"sample template to save it into our database\",\n    \"templateSettings\": ${templateSettings}\n}"

'Param request Data'
KeywordUtil.logInfo('Data request in body: ' + data)


'When Call the request to create new email template with blank token'
ResponseObject res1 = CustomKeywords.'resources.api.buildApiPostMethod'('api/v1/email/template/create', '', data)

'Then verify response code should be 401'
WS.verifyResponseStatusCode(res1, 401)

'When Call the request to create new email template with invalid token'
ResponseObject res2 = CustomKeywords.'resources.api.buildApiPostMethod'('api/v1/email/template/create', 'Bearer', data)
