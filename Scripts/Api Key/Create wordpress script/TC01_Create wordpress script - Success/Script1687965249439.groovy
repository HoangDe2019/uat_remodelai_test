...
'When Call the request to create a new wordpress script'
ResponseObject res = CustomKeywords.'resources.api.buildApiPostMethod'('api/v1/wordpress', token, data)
...
WS.verifyElementPropertyValue(res, "name", paramData.getValue("name", numberRecordInRows) + randomString)
WS.verifyElementPropertyValue(res, "title", paramData.getValue("title", numberRecordInRows) + randomString)
WS.verifyElementPropertyValue(res, "avatarUrl", paramData.getValue("avatarUrl", numberRecordInRows))
WS.verifyElementPropertyValue(res, "companyId", companyId[responseBodyOfComapanies.size()-1])
WS.verifyElementPropertyValue(res, "buttonName", paramData.getValue("buttonName", numberRecordInRows) + randomString )
WS.verifyElementPropertyValue(res, "buttonColor", paramData.getValue("buttonColor", numberRecordInRows))
