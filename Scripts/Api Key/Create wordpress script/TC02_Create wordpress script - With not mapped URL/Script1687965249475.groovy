...
'When Call the request to create a new wordpress script with URL not mapped'
ResponseObject res = CustomKeywords.'resources.api.buildApiPostMethod'('api/'+ randomString + '/wordpress', token, data)

'Then verify response code should be 404'
WS.verifyResponseStatusCode(res, 404)

def responseBodyWorkPress = jsonSlurper.parseText(res.getResponseBodyContent())

'And Verify that the list contains at least one item'
assert !responseBodyWorkPress.empty : 'No content in body!'

'Print the information for each item in the list'
responseBodyWorkPress.each { key, value ->
