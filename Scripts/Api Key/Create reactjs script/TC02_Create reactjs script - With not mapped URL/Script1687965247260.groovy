...
'When Call the request to create a new reactjs script with URL not mapped'
ResponseObject res = CustomKeywords.'resources.api.buildApiPostMethod'('api/vmot/reactjs', token, data)

'Then verify response code should be 404'
WS.verifyResponseStatusCode(res, 404)

def responseBodyReactJS = jsonSlurper.parseText(res.getResponseBodyContent())

'And Verify that the list contains at least one item'
assert !responseBodyReactJS.empty : 'No content in body!'

'Print the information for each item in the list'
responseBodyReactJS.each { key, value ->
