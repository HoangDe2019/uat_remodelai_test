...
'When Call the request to get a chatbot for a given company by companyID with companyinvalid'
...
'When Call the request to get chatbot by chatbotId  with empty token'
def responseChatbot2 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v2/chatbot/${chatbotObj}?token=", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseChatbot2, 401)

'When Call the request to get chatbot by chatbotId with unauthorized'
def responseChatbot3 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v2/chatbot/${chatbotObj}?${paramRequestURI}test", '', '')
