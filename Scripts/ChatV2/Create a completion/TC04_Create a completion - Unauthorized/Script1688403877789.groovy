...
def paramURI1 = ["companyId": itemlist[itemlist.size()-1].companyId, "token": null]

def paramRequestURI1 = CustomKeywords.'resources.api.http_build_query'(paramURI1, '', '&', 'UTF-8').replace(' ', '')
KeywordUtil.logInfo('Data request URL: ' + paramRequestURI1)

'When Call the request to create a completions with blank token'
ResponseObject res1 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v2/chat/completions?${paramRequestURI1}", token, data)
'Then verify response code should be 401'
WS.verifyResponseStatusCode(res1, 401)

def paramURI2 = ["companyId": itemlist[itemlist.size()-1].companyId, "token": "Bearer"]
def paramRequestURI2 = CustomKeywords.'resources.api.http_build_query'(paramURI2, '', '&', 'UTF-8').replace(' ', '')
KeywordUtil.logInfo('Data request URL: ' + paramRequestURI2)

'When Call the request to create a completions with invalid token'
ResponseObject res2 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v2/chat/completions?${paramRequestURI2}", token, data)
'Then verify response code should be 401'
