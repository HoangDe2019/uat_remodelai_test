...
'When Call the request to get a lead form by leadId with blank token'
def formId = responseBodyOfForm[0].leadFormId
ResponseObject responseFormDetail1 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/leadForm/' + formId + '?token=', 'Bearer', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseFormDetail1, 401)

'When Call the request to get a lead form by leadId with invalid token'
ResponseObject responseFormDetail2 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/leadForm/' + formId + '?' + paramRequestURI +'invaild', '', '')
