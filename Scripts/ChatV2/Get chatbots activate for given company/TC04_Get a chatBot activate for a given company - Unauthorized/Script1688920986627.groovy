...
def paramRequestURI = CustomKeywords.'resources.api.http_build_query'(paramURI, '', '&', 'UTF-8').replace(' ', '') + 'test'
KeywordUtil.logInfo('Data request URL: ' + paramRequestURI)

'When Call the request to get chatbots active for a company with unauthorized'
ResponseObject responseChatBotCompany1 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/chatbot/company/activate/' + companyId + '?token=', '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseChatBotCompany1, 401)

'When Call the request to get chatbots active for a company with unauthorized'
ResponseObject responseChatBotCompany2 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/chatbot/company/activate/' + companyId + '?' + paramRequestURI, '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseChatBotCompany2, 401)
