...
'When Call the request to get a chatbot active for a given company by companyID with URL not mapped'
ResponseObject responseChatbots = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/chatbot/company/activateinvalid/' + companyId + '?' + paramRequestURI, token, '')

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseChatbots, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseChatbots, "error", 'Not Found')
