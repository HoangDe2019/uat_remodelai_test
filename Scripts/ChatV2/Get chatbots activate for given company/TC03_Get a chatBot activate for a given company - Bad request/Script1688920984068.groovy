...
def companyId = itemlist[itemlist.size()-1].companyId  + '0123'
...
'When Call the request to get a chatbot active for a given company with invalid companyID'
ResponseObject responseChatbots = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/chatbot/company/activate/' + companyId + '?' + paramRequestURI, token, '')

'Then verify response code should be 400'
WS.verifyResponseStatusCode(responseChatbots, 400)
