...
def templateId = elmTempls[elmTempls.size()-1].templateId
...
'When Call the request to get template by templateId'
ResponseObject responseTemplate = CustomKeywords.'resources.api.buildApiGetMethod'("api/v2/template/${templateId}?${paramRequestURI}", token, '')
...
WS.verifyElementPropertyValue(responseTemplate, "companyId", itemlist[itemlist.size()-1].companyId)
WS.verifyElementPropertyValue(responseTemplate, "templateId", templateId)
assert responseBodyTemplate.displayName != null
assert responseBodyTemplate.singleOptionQuestions != null
assert responseBodyTemplate.multipleChoiceQuestions != null
WS.verifyElementPropertyValue(responseTemplate, "userId", itemlist[itemlist.size()-1].userId)
WS.verifyElementPropertyValue(responseTemplate, "email", itemlist[itemlist.size()-1].email)