...
'When Call the request to get template by templateId with URL not mapped'
ResponseObject responseTemplate = CustomKeywords.'resources.api.buildApiGetMethod'("api/vhai/template/${templateId}?${paramRequestURI}", token, '')

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseTemplate, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseTemplate, "error", 'Not Found')
