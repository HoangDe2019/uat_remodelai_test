...
'When Call the request to get template by templateId with blank token'
ResponseObject responseTemplate1 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v2/template/${templateId}?token=", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseTemplate1, 401)

'When Call the request to get template by templateId with invalid token'
ResponseObject responseTemplate2 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v2/template/${templateId}?${paramRequestURI}invaild", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseTemplate2, 401)