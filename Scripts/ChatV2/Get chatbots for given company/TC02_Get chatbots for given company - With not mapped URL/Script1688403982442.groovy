...
'When Call the request to get a chatbot for a given company by companyID with URL not mapped'
ResponseObject responseChatBotCmpany = CustomKeywords.'resources.api.buildApiGetMethod'('api/v2/chatbot/companyinvalid/' + companyId + '?' + paramRequestURI, tokenId, '')

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseChatBotCmpany, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseChatBotCmpany, "error", 'Not Found')