...
'When Call the request to save a lead form response with URL not mapped'
ResponseObject responseFormResponse = CustomKeywords.'resources.api.buildApiPostMethod'('api/v2mot/leadFormResponse/token=' + auToken, token, data)

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseFormResponse, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseFormResponse, "error", 'Not Found')