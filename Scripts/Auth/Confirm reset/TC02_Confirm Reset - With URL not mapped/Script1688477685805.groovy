...
'When Call the request to reset pass system with URL invalid'
def responsePassReset = CustomKeywords.'resources.api.buildApiPostMethod'('confirm_reset_Invalid?' +  paramRequest, token, dataBody1)

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responsePassReset, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responsePassReset, "error", 'Not Found')