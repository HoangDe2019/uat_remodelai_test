...
'When Call the request to update lead form setting with blank token'
ResponseObject res1 = CustomKeywords.'resources.api.buildApiPutMethod'('api/v1/leadForm/leadFormSetting/update/' + itemlist[itemlist.size()-1].leadFormId, '', data)

'Then Verify response code should be 401'
WS.verifyResponseStatusCode(res1, 401)

'When Call the request to update lead form setting with invalid token'
ResponseObject res2 = CustomKeywords.'resources.api.buildApiPutMethod'('api/v1/leadForm/leadFormSetting/update/' + itemlist[itemlist.size()-1].leadFormId, 'Bearer', data)
