...
'When Call the request to get lead forms of a company with blank token'
def companyId = responseBodyOfUsers[0].companyId
ResponseObject responseFormDetail1 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v1/leadForm/company/' + companyId, '', '')

' Then verify response code should be 401'
WS.verifyResponseStatusCode(responseFormDetail1, 401)

'When Call the request to get lead forms of a company with invalid token'
ResponseObject responseFormDetail2 = CustomKeywords.'resources.api.buildApiGetMethod'('api/v1/leadForm/company/' + companyId, 'Bearer', '')
