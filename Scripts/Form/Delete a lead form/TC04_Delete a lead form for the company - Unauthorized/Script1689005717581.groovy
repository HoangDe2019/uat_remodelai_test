...
def leadFormId = responseBodyOfForm[responseBodyOfForm.size() - 1].leadFormId

'When Call the request to delete a lead form by leadFormId with blank token'
def responseLeadForm1 = CustomKeywords.'resources.api.buildApiDeleteMethod'("api/v1/leadForm/delete/" + leadFormId, '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseLeadForm1, 401)

'When Call the request to delete a lead form by leadFormId with invalid token'
def responseLeadForm2 = CustomKeywords.'resources.api.buildApiDeleteMethod'("api/v1/leadForm/delete/" + leadFormId, 'Bearer', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseLeadForm2, 401)
