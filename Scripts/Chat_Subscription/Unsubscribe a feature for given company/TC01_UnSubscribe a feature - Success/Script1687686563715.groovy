...
for (int i = 1; i <= paramData.getRowNumbers(); i++) {
	if(i == numberRecordInRows) {
		...
		data = "{\r\n    \"topic\": \"${paramData.getValue('topic', i)} - ${randomString}\",\r\n    \"choice\": \"${paramData.getValue('choice', i)} - ${randomStringNumber}\",\r\n    \"companyId\": \"${itemlist[itemlist.size()-1].companyId}\"\r\n}"
		break
	}
}
...
'When Call the request to unsubscribe a feature'
ResponseObject res = CustomKeywords.'resources.api.buildApiPostMethod'('api/v1/unsubscribe', token, data)
...
WS.verifyElementPropertyValue(res, "choice", paramData.getValue('choice', numberRecordInRows))
WS.verifyElementPropertyValue(res, "topic", paramData.getValue('topic', numberRecordInRows))
