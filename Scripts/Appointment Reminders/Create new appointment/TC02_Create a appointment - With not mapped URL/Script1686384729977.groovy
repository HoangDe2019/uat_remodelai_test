...
for (int i = 1; i <= paramData.getRowNumbers(); i++) {
	if(i == numberRecordInRows) {
		...
		data = [
			"name" : paramData.getValue("name", i), 
			"phoneNumber": paramData.getValue("phoneNumber", i),
			"delta": paramData.getValue("delta", i), // delta is integer only have value from 5 until max 60 mins
...
'When Call the request to create a new appointment with URL not mapped'
ResponseObject res = CustomKeywords.'resources.api.buildApiPostMethodByFormData'("createinvalid", '', data)
