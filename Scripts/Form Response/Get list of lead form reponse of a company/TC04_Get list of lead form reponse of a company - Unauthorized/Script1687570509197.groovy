...
'When Call the request to get listFormResponse by companyID with blank token'
def companyId = itemlist[itemlist.size()-1].companyId
ResponseObject responseFormDetail1 = CustomKeywords.'resources.api.buildApiGetMethod'('/api/v1/leadFormResponse/company/' + companyId, '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseFormDetail1, 401)

'When Call the request to get listFormResponse by companyID with invalid token'
ResponseObject responseFormDetail2 = CustomKeywords.'resources.api.buildApiGetMethod'('/api/v1/leadFormResponse/company/' + companyId, 'Bearer', '')
