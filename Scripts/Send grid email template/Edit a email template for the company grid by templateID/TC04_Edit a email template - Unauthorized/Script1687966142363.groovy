...
'When Call the request to edit a email template with blank token'
ResponseObject res1 = CustomKeywords.'resources.api.buildApiPostMethod'('api/v1/sendgrid/email/template/' + templateId + '/edit', '', data)

'Then verify response code should be 401'
WS.verifyResponseStatusCode(res1, 401)

'When Call the request to edit a email template with invalid token'
ResponseObject res2 = CustomKeywords.'resources.api.buildApiPostMethod'('api/v1/sendgrid/email/template/' + templateId + '/edit', 'Bearer', data)
