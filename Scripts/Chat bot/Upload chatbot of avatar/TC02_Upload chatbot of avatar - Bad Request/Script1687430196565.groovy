...
'When Call the request to update chatbot avatar with invalid chatbotId'
def responseChatbotAvatar1 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot/${responseBodyOfChatbots[responseBodyOfChatbots.size()-1].chatBotId}test/avatar", token, dataRequest)

'Then verify response code should be 400'
WS.verifyResponseStatusCode(responseChatbotAvatar1, 400)

def dataInvalid = "{'0000000000000000000'}"

'When Call the request to update chatbot avatar with invalid data request'
def responseChatbotAvatar2 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot/${responseBodyOfChatbots[responseBodyOfChatbots.size()-1].chatBotId}/avatar", token, dataInvalid)

'Then verify response code should be 400'
WS.verifyResponseStatusCode(responseChatbotAvatar2, 400)