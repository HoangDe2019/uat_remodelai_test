...
'When Call the request to update chatbot avatar with invalid URL'
def responseChatbotAvatar = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot1/${responseBodyOfChatbots[responseBodyOfChatbots.size()-1].chatBotId}test/avatar", token, dataRequest)

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseChatbotAvatar, 404)