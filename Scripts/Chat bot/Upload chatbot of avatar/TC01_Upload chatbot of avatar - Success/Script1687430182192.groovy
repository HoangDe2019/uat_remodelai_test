...
'When Call the request to update chatbot avatar'
def responseChatbotAvatar = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot/${responseBodyOfChatbots[responseBodyOfChatbots.size()-1].chatBotId}/avatar", token, dataRequest)

'Then verify response code should be 200'
WS.verifyResponseStatusCode(responseChatbotAvatar, 200)

'And Verify response body is properly'
WS.verifyEqual(responseChatbotAvatar.responseText, 'Avatar uploaded successfully.')