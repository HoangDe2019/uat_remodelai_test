...
def responseChatbots1 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/chatbot/company/${itemlist[itemlist.size()-1].companyId}", token, '')
...
def chatbotObj = responseBodyOfChatbots1[responseBodyOfChatbots1.size()-1].chatBotId

//'When Call the request to active chatbot by invalid companyId'
//def responseChatbot1 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot/null/activate/${chatbotObj}", token, '')
//
//'Then verify response code should be 400'
//WS.verifyResponseStatusCode(responseChatbot1, 400)

'When Call the request to active chatbot by invalid chatbotId'
def responseChatbot2 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot/${itemlist[itemlist.size()-1].companyId}/activate/000", token, '')

'Then verify response code should be 400'
WS.verifyResponseStatusCode(responseChatbot2, 400)
