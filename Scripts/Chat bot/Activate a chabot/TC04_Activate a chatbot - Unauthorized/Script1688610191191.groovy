...
'When Call the request to active chatbot with blank token'
def responseChatbot2 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/chatbot/${itemlist[itemlist.size()-1].companyId}/activate/${chatbotObj}", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseChatbot2, 401)

'When Call the request to active chatbot with invalid token'
def responseChatbot3 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/chatbot/${itemlist[itemlist.size()-1].companyId}/activate/${chatbotObj}", 'Bearer', '')
