...
'When Call the request to active chatbot by chatbotId and company with URL not mapped'
def responseChatbot2 = CustomKeywords.'resources.api.buildApiPostMethod'("api/v1/vmot/chatbot/${itemlist[itemlist.size()-1].companyId}/activate/${chatbotObj}", token, '')

'Then verify response code should be 404'
WS.verifyResponseStatusCode(responseChatbot2, 404)

'And Verify response body is properly'
WS.verifyElementPropertyValue(responseChatbot2, "error", 'Not Found')