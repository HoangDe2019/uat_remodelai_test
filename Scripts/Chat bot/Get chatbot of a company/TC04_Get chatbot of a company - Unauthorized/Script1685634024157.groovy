...
'When Call the request to get chatbot of a company with blank token'
def responseChatbots1 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/chatbot/company/${itemlist[0].companyId}", '', '')

'Then verify response code should be 401'
WS.verifyResponseStatusCode(responseChatbots1, 401)

'When Call the request to get chatbot of a company with invalid token'
def responseChatbots2 = CustomKeywords.'resources.api.buildApiGetMethod'("api/v1/chatbot/company/${itemlist[0].companyId}", 'Bearer', '')
